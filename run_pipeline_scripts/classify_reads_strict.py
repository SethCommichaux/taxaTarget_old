...
parser.add_argument("-mg", help="214 marker gene name file")
...
mg_names = [i.strip() for i in open(args.mg)]
mg_names += ['Total_reads']

sample_taxa = {'names':mg_names}
taxa2bitscores = {'names':mg_names}
...
for read,v in read_classifications.items():
    ...
    if line not in sample_taxa:
        sample_taxa[line] = np.zeros(215)
        taxa2bitscores[line] = np.zeros(214)
    for i in buscos:
        sample_taxa[line][mg_names.index(i)] += 1
        sample_taxa[line][214] += 1
        taxa2bitscores[line][mg_names.index(i)] += mean_bitscore
