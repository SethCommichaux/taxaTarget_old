...
def build_taxa_dict():
	taxaID2_Name_Lineage,names = {},{}
	for i in open(args.t):
		tmp = [j.strip() for j in i.lower().split('|')]
		taxaID = tmp[0]
		taxaName = tmp[1]
		lineage = tmp[2]
	...
	return taxaID2_Name_Lineage,names

nodes = {}

for i in open(args.no):
	tmp = [j.strip() for j in i.split('|')]
	id,rank = tmp[0],tmp[2]
	if rank == 'species':
		nodes[id] = rank

taxaID2_Name_Lineage,names = build_taxa_dict()
