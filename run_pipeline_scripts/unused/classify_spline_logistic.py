...
def subset_reads():
    ...
    for i in open(args.d):
        ...
        if aln_len >= 30:
            ...
            elif bitscore == results[read][0]:
                results[read][1].append(tmp)
...
def LCA(lineages):
    ...
    ''' # tends towards predicting more specific ranks when there is tie between classifiers of different taxonomic ranks
    nr_lineages = []
...
with open('Taxonomic_report.txt','w') as out:
    ...
    for query,line_genes in best_probs.items():
        ...
        if len(diamond_subset[query][1]) == 1: final_results[query] = ';'.join(lca)+'\tsingleton' 
	if len(diamond_subset[query][1]) > 1: final_results[query] = ';'.join(lca)+'\tmultimapped'
        if lca == []: continue
        line = ''
        for i in lca:
            if i == '': continue
    ...
    for k,v in sorted(taxa_counts.items()):
        ...
        if mg_counts < 0.10*taxa_prot_count:
            if float(read_counts)/mg_counts > 5.0:
                line = ''
                for i in k.split(';'):
                    line += i+';'
                    if line in taxa_counts:
                        taxa_counts[line][0] -= read_counts
    for k,v in sorted(taxa_counts.items()):
        read_counts = v[0]
        mg_counts = len(set(v[1]))
        taxa_prot_count = busco2protlen[names[k.split(';')[-2]]][1]
